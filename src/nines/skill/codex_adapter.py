"""Codex-specific skill adapter with filesystem generation.

Provides :class:`CodexAdapter` with a high-level
:meth:`generate_skill_dir` method that creates the
``.codex/skills/nines/`` directory containing ``SKILL.md`` and
...
from nines.skill.adapters import CodexAdapter as _BaseCodexAdapter
from nines.skill.manifest import SkillManifest

logger = logging.getLogger(__name__)


class CodexAdapter(_BaseCodexAdapter):
    """Extended Codex adapter that writes skill files to disk.
    ...
    def generate_skill_dir(
        ...
        """Create ``.codex/skills/nines/`` with SKILL.md and commands.

        Parameters
        ----------
        output_path:
            Project root directory.  The ``.codex/skills/nines/``
