...
NineS {{ nines_version }} 还带来了**多运行时技能支持**（通过 `nines install --target` 支持 Cursor、Claude Code、Codex 和 Copilot）以及**自升级能力**——值得一提的是，Caveman 的跨 IDE 规则同步方案现在可以与 NineS 自身的跨 IDE 安装机制进行对照分析。
...
!!! abstract "{{ nines_version }} 演进"
    NineS {{ nines_version }} 在可执行评测方法论之上，新增 **19 维自评体系**（D01–D19）、**EvoBench 32 维集成**、**多运行时技能安装**（通过 `nines install --target` 支持 Cursor、Claude Code、Codex、Copilot）以及通过 `nines update` 实现的**自升级能力**。关于 Caveman 有效性的每一个判断都有评测数据支撑——而 NineS 现在也通过完整的 MAPIM 自迭代循环，以同样的严谨标准衡量自身。
...
!!! abstract "亲自运行"
    ...
    # 分析完成后验证 NineS 自身的能力维度
    nines self-eval
    ```

    运行 NineS 的面向 Agent 分析，亲身体验机制分解、关键点提取、基准评测和映射表——而不仅仅是文件计数和复杂度评分。分析完成后可运行 `nines self-eval`，检查 NineS 自身的 19 维评估结果，确认其分析能力已校准。
